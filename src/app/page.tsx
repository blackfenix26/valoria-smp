...
export default async function Home(){
  ...
  const [t1,t2] = (banner?.title ?? cfg.hero_title ?? "Produtos para uma experiência inesquecível no | Valoria BLCKX.").split("|");
  ...
  return <main>
    <section className="relative min-h-[520px] overflow-hidden border-b border-white/10" style={{backgroundImage:`linear-gradient(90deg,#03050Af2 0%,#03050Ab8 38%,#03050A22 100%),url(${hero})`,backgroundSize:"cover",backgroundPosition:"center"}}>
      ...
      <div className="relative mx-auto flex min-h-[520px] max-w-[1400px] items-center px-4 py-20 lg:px-6"><div className="max-w-2xl">
        <span className="inline-flex rounded-full border border-vio/60 bg-black/30 px-4 py-2 text-[11px] font-bold tracking-wider text-white backdrop-blur">★ O UNIVERSO DE VALORIA BLCKX</span>
        <h1 className="mt-5 text-4xl font-black leading-[1.02] md:text-6xl">{t1}{t2&&<span className="grad">{t2}</span>}</h1>
        <p className="mt-5 max-w-xl text-base leading-7 text-white/70">{banner?.description ?? cfg.hero_subtitle ?? "Explore produtos, vantagens e experiências exclusivas para sua jornada em Valoria BLCKX."}</p>
        <div className="mt-7 flex flex-wrap gap-3"><Link href={banner?.link||"/atualizacoes"} className="btn-p">{banner?.buttonText||"Conhecer Valoria BLCKX"}</Link><Link href="/produtos" className="btn-o">Explorar produtos</Link></div>
      </div></div>
    </section>
    ...
    <div className="mx-auto max-w-[1400px] space-y-12 px-4 py-8 lg:px-6">
      ...
      <section id="produtos"><div className="flex flex-wrap items-end justify-between gap-4"><div><h2 className="text-2xl font-black md:text-3xl">Produtos oficiais da Valoria BLCKX</h2><p className="mt-1 text-sm text-mut">Encontre tudo o que precisa para sua experiência em Valoria BLCKX.</p></div><Link href="/produtos" className="text-sm font-semibold text-vio2">Ver todos os produtos →</Link></div>
